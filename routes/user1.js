...
function buildData(reqData,userInfo){
	...
	var ydelta = tools.GetDateDiff(new Date("1924/2/4"), date, "year");
    
	bigyun = ((Math.floor(ydelta / 60) + 1) % 9) + 1;
	smallyun = ((Math.floor(ydelta / 20) + 3) % 9) + 1;

    //年运数-- 1924年2月4日年运4
	nianyun = 8 - ((ydelta + 5) % 9) + 1;
	//月运数
	//首先判断他在第几月
    ...
    //时运数++从1924年2月4日01:00起 5
	var cdelta = tools.GetDateDiff(new Date("1924/2/4 01:00:00"), date, "hour");
	shiyun = ((Math.floor(cdelta / 2) + 4) % 9) + 1;
