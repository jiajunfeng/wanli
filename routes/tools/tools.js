...
function toFixed(n,m){ //用于flash
  var f='', p=Math.pow(10,m); //p为10进制移位量;
  if(n<0) n = -n, f = '-';   //把负数转为正数
  var a=Math.floor(n),b=n-a; //分离整数与小数
  b = Math.round(b*p );      //移位并四舍五入
  if(b>=p) a++,b-=p;         //进位
...
var cs_k = 0.2725076; //月亮与地球的半径比
var cs_k_nasa = 0.2722768;
var cs_sMoon = cs_k*cs_rEar*1.0000036*rad; //用于月亮视半径计算
var cs_sMoon_nasa = cs_k_nasa*cs_rEar*1.0000036*rad; //用于月亮视半径计算
//var cs_sMoon_nasa = 358204545;
...
function m2fm(v,fx,fs){//秒转为分秒,fx为小数点位数,fs为1转为"分秒"格式否则转为"角度分秒"格式
 ...
 if(!fs)   return gn + f + "'"  +toFixed(m,fx) + '"' ;
 if(fs==1) return gn + f + '分' +toFixed(m,fx) + '秒';
 if(fs==2) return gn + f + 'm'  +toFixed(m,fx) + 's';
}
...
var JD={ //日期元件
  ...
  JD2str:function(jd){ //JD转为串
   if( !this.bufD ) this.bufD = new Object();
   this.DD( jd,this.bufD );
   return this.DD2str( this.bufD );
  },


  Y:2000, M:1, D:1, h:12, m:0, s:0,
  toJD:function(){ return this.JD( this.Y, this.M, this.D+((this.s/60+this.m)/60+this.h)/24 ); }, //公历转儒略日
  setFromJD:function(jd){  this.DD(jd,this);  }, //儒略日数转公历
...
function nutation(t,zq){ //章动计算,t是J2000.0起算的儒略世纪数,zq表示只计算周期天于zq(天)的项
...
S_aLon:function(t,n){  //太阳视黄经
 return this.E_Lon(t,n) + ZB.nutationLon(t) + gxc_sunLon(t) + Math.PI; //注意，这里的章动计算很耗时
},
...
var touY={
 ...
 toxy0:function(J,W,a){ //平面正投
  ...
  var x=cos(W)*cos(J), y=cos(W)*sin(J), z=sin(W);
  a.x = x;
  a.y = this.cosE0*y + this.sinE0*z;
  a.z =-this.sinE0*y + this.cosE0*z;
 },
 toxy1:function(J,W,a){//横轴等距投影
  J -= this.J0;
  var L = atan2( sin(W),cos(W)*sin(J) );
  var B = acos ( cos(W)*cos(J) );
  a.x = a.y = 0, a.z=-1;
  if(B>Math.PI-0.1) return; //接近180度失真太大，不必计算
  B = B/Math.PI;
  a.x = B*cos(L), a.y = B*sin(L), a.z = 1;
 },
 toxy2:function(J,W,a){//横轴等积投影
  J -= this.J0;
  var L = atan2( sin(W),cos(W)*sin(J) );
  var B = acos ( cos(W)*cos(J) );
  a.x = a.y = 0, a.z=-1;
 ...
 toxy4:function(J,W,a){ //摩尔威特
  ...
  a.y = this.MollCZ(W);
  a.x = sqrt(1-a.y*a.y)*J*2/Math.PI;
  a.z = 1;
 },
 ...
 toxy8:function(J,W,a){ //中国灯笼投影
  J = rad2rrad(J-this.J0);
  var C = cos(W*2/3);     //纬线长度
  var v = sin(W*2/3)/5.3; //纬线曲率
...
function Lunar(){
 ...
 this.yueLiCalc=function(By,Bm){
  ...
  for(i=0,j=0;i<Bdn;i++){
    ...
    D = ob.Lyear +9000;  ob.Lyear2 = obb.Gan[D%10]+obb.Zhi[D%12]; //干支纪年(立春)
    D = ob.Lyear0+9000;  ob.Lyear3 = obb.Gan[D%10]+obb.Zhi[D%12]; //干支纪年(正月)
...
function GetDateDiff(sTime, eTime, diffType) {
    ...
    switch (diffType) {
        ...
        case "year":
            {
                var delta = eTime.getFullYear() - sTime.getFullYear();
                if(eTime.getMonth() < sTime.getMonth() || (eTime.getMonth() == sTime.getMonth() && eTime.getDate() < sTime.getDate())){
                    delta = delta - 1;
                }
                return delta;
            }
...
exports.timeStr2hour=timeStr2hour;
exports.JD=JD;
exports.obb=obb;
exports.J2000 =J2000;
exports.year2Ayear =year2Ayear;
exports.nianLiHTML = nianLiHTML;
